...
sm_ag_if_ans_t write_agent(const sm_ag_if_wr_t* data)
{
  ...
  assert(data->type == SUBSCRIBE_TIMER  
         || data->type == PDCP_REQ_CTRL_V0 );

  if(data->type == SUBSCRIBE_TIMER){
    printf("SUBSCRIBE_TIMER called with ms = %u \n", data->sub_timer.ms);
    sm_ag_if_ans_t ans = {.type = PDCP_AGENT_IF_SUBS_REQ_V0 };
    return ans;
  } 

  assert(data->type == PDCP_REQ_CTRL_V0 );
...
void check_control(sm_agent_t* ag, sm_ric_t* ric)
{
  ...
  sm_ag_if_wr_t wr = {.type = PDCP_REQ_CTRL_V0, .pdcp_req_ctrl = data }; 
